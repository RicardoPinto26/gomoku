...
import pt.isel.leic.daw.gomokuRoyale.services.lobby.LobbyDetailsError
...
fun LobbyServicesError.toResponse() = when (this) {
    is LobbyCreationError -> when (this) {
        ...
        is LobbyCreationError.UserAlreadyInALobby ->
            Problem.response(Problem.userAlreadyInALobby(this.lobbyID, this.gameID))
    }
    ...
    is LobbySeekError -> when (this) {
        is LobbySeekError.UserAlreadyInALobby ->
            Problem.response(Problem.userAlreadyInALobby(this.lobbyID, this.gameID))
    }
    ...
    is LobbyDetailsError -> when (this) {
        LobbyDetailsError.LobbyNotFound -> Problem.response(Problem.lobbyNotFound)
    }
}
